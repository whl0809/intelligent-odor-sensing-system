...
The model is hierarchical:
  1. blank detector: blank / food
  2. food type: banana / meat
  3. food-specific freshness: fresh / not_fresh
  4. output mapping: banana+not_fresh -> fermented; meat+not_fresh -> spoiled
...
def train(
    ...
    # Compatibility fields allow older test scripts to load this bundle.  New
    # inference should use blank_model, food_subtype_model, and condition_models.
    legacy_food_model = make_elastic_logistic().fit(table[features], table["food"])
    ...
    report = {
        ...
        "model_architecture": (
            "blank-vs-food elastic logistic -> banana-vs-meat elastic logistic -> "
            "banana Extra Trees or meat elastic logistic freshness"
        ),
...
def predict(model_path: Path, csv_path: Path, baseline_csv: Path | None) -> dict:
    ...
    if bundle.get("format_version", 1) >= 2 and "blank_model" in bundle:
    ...
    condition_X = X
    if bundle.get("format_version", 1) >= 2 and "condition_models" in bundle:
    ...
    if baseline_csv is not None:
        ...
        if bundle.get("format_version", 1) < 2 and bundle.get("condition_baseline_model") is not None:
