...
Default repository layout:
    ...
    ECE450_software/
    ├── config/rpi5.toml
    ├── config/co5300_init.json
    ├── model/test_model.py
    ├── model/classification_outputs/enose_multitask_model.joblib
    ├── src/enose/
    └── tools/co5300_dashboard.py
...
window length, then repeats whenever the rolling window advances.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--no-baseline", action="store_true", help="Use absolute freshness model")
...
def resolve_paths(root: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "model": output / "enose_multitask_model.joblib",
...
def acquisition_command(paths: dict[str, Path], frames: int) -> list[str]:
    return [
        sys.executable,
        "-m",
        "enose",
...
def acquire(paths: dict[str, Path], frames: int) -> Path:
    if frames < 80:
    ...
    # The current model requires SVM41 temperature/RH/VOC/NOx, so the full
    # acquisition command is mandatory. The older no-SGP41 command is invalid.
    command = acquisition_command(paths, frames)
...
def publish_live_prediction(
    ...
    *,
    ...
    if not args.no_display:
        # Draw one frame and return immediately so acquisition can continue.
        show_dashboard(paths, args, hold=False)
    return result
...
def acquire_with_live_predictions(
    ...
    print(
        f"Live prediction starts at {first_prediction} valid frames and refreshes "
    ...
    try:
        if args.input_csv:
            ...
            if args.no_baseline:
                ...
                raise ValueError(
                    "Existing --input-csv requires --baseline-csv or --no-baseline"
                )
            result = run_model(
                paths,
                sample,
            ...
            if args.no_baseline:
                ...
                baseline = acquire(paths, args.baseline_frames)
            if not args.yes:
            ...
            if not args.no_display and not args.no_hold:
                # Acquisition is finished; keep the final live window on-screen.
                show_dashboard(paths, args, hold=True)
